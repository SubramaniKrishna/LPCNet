...
def difflpc(nb_used_features = 38, training=False, use_gpu=True, adaptation=False):
    ...
    L3 = Dense(50, activation='tanh')
    L4 = Dense(lpcoeffs_N, activation='tanh',name = "rc")
    rc = L4(L3(L2(L1(feat))))
    # Differentiable RC 2 LPC
    lpcoeffs = diff_rc2lpc(name = "rc2lpc")(rc)
    preds = diff_pred(name="prediction")([pcm_1,lpcoeffs])

    model = Model([pcm_1,feat], [lpcoeffs,preds])
    model.nb_used_features = nb_used_features
    model.frame_size = frame_size
    return model
